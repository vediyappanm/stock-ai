...
# ── PyTorch classes are only defined when torch is available ──────────────────
if _TORCH_AVAILABLE:
    ...
    class LSTMModel:
        ...
        def train_and_predict(self, df: pd.DataFrame) -> LSTMTrainResult:
            ...
            if not np.isfinite(X).all() or not np.isfinite(y).all():
                logger.warning("LSTM training skipped: data contains non-finite values")
                return LSTMTrainResult(prediction=float(df["Close"].iloc[-1]), residual_std=1.0)
            ...
            with torch.no_grad():
                ...
                latest_seq = torch.from_numpy(
                    self.scaler_x.transform(df[INDICATOR_COLUMNS].tail(settings.lstm_sequence_length).values)
    ...
    # Stub class when torch is unavailable (e.g. cloud lite deployment)
    class LSTMModel:  # type: ignore[no-redef]
        """Stub LSTM model — torch not installed in this environment."""

        def __init__(self) -> None:
            raise ModelError("Torch is not available in this environment.", failed_step="PREDICT_PRICE")

        def train_and_predict(self, df: pd.DataFrame) -> LSTMTrainResult:
            raise ModelError("Torch is not available in this environment.", failed_step="PREDICT_PRICE")

        @classmethod
        def from_checkpoint(cls, path) -> "LSTMModel":
            raise ModelError("Torch is not available in this environment.", failed_step="PREDICT_PRICE")
