...
def fetch_ohlcv_data(
    ...
    # Setup a "Real Browser" session to bypass cloud IP blocks (e.g. Render/GCP)
    ...
    try:
        # Strategy A: Multi-threaded download with human-like session
        logger.info("Attempting yf.download for %s (session: impersonated)", ticker_symbol)
        raw = yf.download(
        ...
        # Strategy B: Individual Ticker history fallback (often uses different API endpoints)
        if raw.empty:
            logger.info("yf.download empty for %s, trying Strategy B: Ticker.history", ticker_symbol)
            ticker_obj = yf.Ticker(ticker_symbol, session=session)
            raw = ticker_obj.history(period=period, interval=interval)
            
    except Exception as exc:
        logger.warning("Strategy A/B failed for %s: %s. Trying legacy fallback...", ticker_symbol, exc)
        try:
            # Strategy C: Raw individual fetch without custom session as a last resort
            ticker_obj = yf.Ticker(ticker_symbol)
            raw = ticker_obj.history(period=period, interval=interval)
        except Exception as inner_exc:
            raise NetworkError(
                f"Market Data Provider Blocked: Failed to fetch {ticker_symbol} after 3 strategies and human impersonation. Error: {inner_exc}",
                failed_step="FETCH_DATA",
            ) from inner_exc
