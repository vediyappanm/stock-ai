...
def fetch_macro_features(start_date: str | None = None, end_date: str | None = None) -> pd.DataFrame:
    """
    Fetch macro features: VIX, 10Y Treasury yield, Fed funds rate.
    Returns DataFrame with Date, VIX, Yield_10Y, FedRate columns.
    """
    if not _HAS_YF:
    ...
    try:
        ...
        # Fed Funds Rate: ^IRX (13-week T-bill as proxy, daily)
        irx = yf.download("^IRX", start=start_date, end=end_date, progress=False)
        irx = irx[["Close"]].rename(columns={"Close": "FedRate"}).reset_index()

        # Merge on Date
        macro = vix.merge(tnx, on="Date", how="outer").merge(irx, on="Date", how="outer")
        macro = macro.sort_values("Date").reset_index(drop=True)

        # Fill gaps with forward fill
        macro[["VIX", "Yield_10Y", "FedRate"]] = macro[["VIX", "Yield_10Y", "FedRate"]].ffill().fillna(0.0)
